...
function calculate() {
    const miniMaple = new (0, _miniMaple.MiniMaple)();
    let res = "";
...
class MiniMaple {
    isValid(str) {
        ...
        str = str.replace(/\s+/g, "");
        if (str.match(/^((\+|\-)((\d+|[a-z])(\*(\d+|[a-z])(\^\d+)?)?|[a-z]\^\d+(\*(\d+|[a-z]))?))+$/g) === null) throw new Error("Expression doesn't match the pattern");
    ...
    diff(str, x) {
        ...
        fullExpression.forEach((term)=>{
            ...
            let indPow = term.match(x + "\\^\\d+") ? term.match(x + "\\^\\d+")[0].slice(2) : null; // transform from x*x -> x^2 or x*x^2 -> x^3
            if (mult1 !== null && mult2 !== null) {
                if (indPow !== null) term = term[0] + x + "^" + (Number(indPow) + 1);
                else term = term[0] + x + "^2";
                mult1 = null;
                mult2 = null;
